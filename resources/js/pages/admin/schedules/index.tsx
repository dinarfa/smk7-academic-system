    ...
    return (
        <>
            ...
            <div className="space-y-6 p-4 sm:p-6 lg:p-8">
                <div>
                    <h1 className="text-2xl font-semibold tracking-tight text-foreground">Jadwal Pelajaran</h1>
                            ...
                            <form onSubmit={handleCreate} className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    {createForm.errors.school_class_id && (
                                        <p className="text-xs text-destructive">{createForm.errors.school_class_id}</p>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    {createForm.errors.schedule_type && (
                                        <p className="text-xs text-destructive">{createForm.errors.schedule_type}</p>
                                    )}
                                </div>
                                ...
                                {createForm.data.schedule_type === 'subject' && (
                                    <div className="space-y-2">
                                        ...
                                        {createForm.errors.subject_id && (
                                            <p className="text-xs text-destructive">{createForm.errors.subject_id}</p>
                                        )}
                                    </div>
                                )}
                                ...
                                <div className="space-y-2">
                                    ...
                                    {createForm.errors.day_of_week && (
                                        <p className="text-xs text-destructive">{createForm.errors.day_of_week}</p>
                                    )}
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-2">
                                        ...
                                        {createForm.errors.starts_at && (
                                            <p className="text-xs text-destructive">{createForm.errors.starts_at}</p>
                                        )}
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        {createForm.errors.ends_at && (
                                            <p className="text-xs text-destructive">{createForm.errors.ends_at}</p>
                                        )}
                                    </div>
                                </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {filteredSchedules.length === 0 ? (
                            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-12">
                                ...
                                <p className="text-sm text-muted-foreground text-center">Belum ada jadwal.<br/>Tambahkan slot dari panel kiri.</p>
                            </div>
                        ) : (
                                    ...
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left text-sm">
                                            <thead className="border-b border-border text-xs font-medium text-muted-foreground">
                                                <tr>
                                                    <th className="px-4 py-3 pl-6">Hari</th>
                                                    <th className="px-4 py-3">Jam</th>
                                                    <th className="px-4 py-3">Kelas</th>
                                                    <th className="px-4 py-3">Tipe Sesi</th>
                                                    <th className="px-4 py-3">Mata Pelajaran</th>
                                                    <th className="px-4 py-3">Guru</th>
                                                    <th className="px-4 py-3 pr-6 text-right">Aksi</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-border">
                                                {[...filteredSchedules]
                                                    ...
                                                    <tr key={slot.id} className="hover:bg-muted/50">
                                                        ...
                                                        <td className="px-4 py-3 whitespace-nowrap">
                                                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${SCHEDULE_TYPE_COLORS[slot.schedule_type] ?? ''}`}>
                                    ...
                                    {Math.ceil(filteredSchedules.length / ITEMS_PER_PAGE) > 1 && (
                                        <div className="flex items-center justify-between border-t border-border px-6 py-4">
                                            <span className="text-xs text-muted-foreground">
                                                Halaman <span className="font-medium text-foreground">{currentPage}</span> dari <span className="font-medium text-foreground">{Math.ceil(filteredSchedules.length / ITEMS_PER_PAGE)}</span>
                                            </span>
                        ...
                        <form onSubmit={handleEdit} className="space-y-4 mt-2">
                            <div className="space-y-2">
                                ...
                                {editForm.errors.school_class_id && (
                                    <p className="text-xs text-destructive">{editForm.errors.school_class_id}</p>
                                )}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {editForm.errors.schedule_type && (
                                    <p className="text-xs text-destructive">{editForm.errors.schedule_type}</p>
                                )}
                            </div>
                            ...
                            {editForm.data.schedule_type === 'subject' && (
                                <div className="space-y-2">
                                    ...
                                    {editForm.errors.subject_id && (
                                        <p className="text-xs text-destructive">{editForm.errors.subject_id}</p>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="space-y-2">
                                ...
                                {editForm.errors.day_of_week && (
                                    <p className="text-xs text-destructive">{editForm.errors.day_of_week}</p>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-2">
                                    ...
                                    {editForm.errors.starts_at && (
                                        <p className="text-xs text-destructive">{editForm.errors.starts_at}</p>
                                    )}
                                </div>
                                <div className="space-y-2">
                                    ...
                                    {editForm.errors.ends_at && (
                                        <p className="text-xs text-destructive">{editForm.errors.ends_at}</p>
                                    )}
                                </div>
                            </div>
